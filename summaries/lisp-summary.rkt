...
(require racket/match
         ...
         "sexp-summary.rkt")       ; sexp-smr (leaf AND combine on its values), sand-spines
...
;;   val    sexp measure of the code-equivalent text under this entry
(struct arm (exit n first tailc pend val) #:transparent)

(define (lisp-leaf s)
  (for/hasheq ([m (in-list modes)])
    (define-values (exit text n first tailc pend) (measure s m))
    (values m (arm exit n (or first (and pend 'hash)) tailc pend (sexp-smr text)))))
...
(define (lisp+ A B)
  (for/hasheq ([m (in-list modes)])
    (match-define (arm m1 na fa ta pa va) (hash-ref A m))
    (match-define (arm m2 nb fb tb pb vb) (hash-ref B m1))
    ...
    (define-values (tc pd)
      (cond [(not fb)       (values ta pa)]            ; B empty
            ...
            [else           (values tb pb)]))
    (values m (arm m2 (+ na nb s) f tc pd (sexp-smr va vb)))))
...
;; from 'code.
(define (lisp-state L) (arm-exit (hash-ref L 'code)))
...
(define (lisp-spines L R)
  (match-define (arm m _ _ _ _ vL) (hash-ref L 'code))
  (sand-spines vL (arm-val (hash-ref R m))))
...
(define (class-sides L R)
  (match-define (arm m _ _ tc pd _) (hash-ref L 'code))
  (values (if pd 'hash tc)
          (arm-first (hash-ref R m))))
  ...
  (define lisp-runs-guide*
    (make-guide* lisp-smr
      (lambda (bs fsl fsr as)
        ...
        (define m0 (arm-exit (hash-ref bs 'code)))
        (match-define (arm m1 nl _ tl pl _) (hash-ref l m0))
        (match-define (arm m2 nr _ tr pr _) (hash-ref r m1))
        ...
        (define (as-first m) (arm-first (hash-ref as m)))
        (define (resolve f m) (if (or (not f) (eq? f 'hash)) (or (as-first m) 'hash) f))
        (define rl (resolve (arm-first (hash-ref r m1)) m2))   ; class right of the seam
...
(module+ test
  (require rackunit)
  (define (code-arm s) (hash-ref (lisp-smr s) 'code))
  (define (code-val s) (arm-val (code-arm s)))     ; the sexp value, entered in code
