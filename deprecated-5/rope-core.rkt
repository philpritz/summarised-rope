...
;; empty, since concat drops empties before branching, so a branch is never empty. It
;; is constant per algebra, so it is built once and cached, keyed by smr; every empty
;; (including leaf-rope on "") returns that one object.
(define empty-leaves (make-weak-hasheq))
(define (empty-rope smr)
  (hash-ref! empty-leaves smr (lambda () (leaf (smr "") smr 0 0 ""))))
(define ((leaf-rope smr) text)
  (if (string=? text "")
      (empty-rope smr)                              ; the one cached empty
...
;; ---------- join ----------
;; joiner: the binary join -- the rope analog of `combine`. Drops empties, fuses the
;; seam when the two boundary leaves fit one leaf, else branches. It's the inverse of
;; bisect's split-leaf: descent splits a leaf, the rise's joins fuse it back. To reach
;; the seam it descends a boundary edge only through SMALL LEAF tips (< max-leaf chars
;; -- the bounded fragment splitting leaves) and stops at any real sub-rope, so it stays
;; O(1)-ish, never O(depth). Char count is read straight off those boundary leaves
;; (string-length is O(1)) -- the only place it is needed, now that nodes cache leaf
;; count, not char size. Balance-dumb: shape is bisect's job. Lifted out of concat-rope
;; so the variadic concat and make-rope fold the SAME binary op, as smr folds combine.
(define (joiner smr)
...
;; concat-rope: the variadic join -- fold the binary `join` from the empty rope.
;; foldr (the empty is the right identity), so the parts keep their order.
(define ((concat-rope smr) . ropes)
  (foldr (joiner smr) (empty-rope smr) ropes))
...
;; ropes (passed through) by mapping `coerce` then folding the binary join, and
;; `rebalance`s if the result came out pathologically tall (a fresh load folds to a
;; right-leaning spine). Mirrors make-summary: map coerce, then fold from the identity.
(define ((make-rope smr) . parts)
  (define mt   (empty-rope smr))                  ; the cached identity rope
  (define join (joiner smr))                      ; the binary op -- concat-rope's join
  (define (coerce x)                              ; a part -> a rope (the summary coerce's twin)
    (match x
      [""          mt]                            ; empty string -> cached empty rope
      [(? string?) (foldr join mt (map (leaf-rope smr) (chunk-string x max-leaf)))]
      [_           x]))                           ; a rope (by contract) passes through
  (define t (foldr join mt (map coerce parts)))   ; map coerce, then fold -- mirrors smr
