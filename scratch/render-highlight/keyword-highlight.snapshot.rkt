...
;; SCRATCH -- keyword highlighter on the summarised rope: keywords + string awareness +
;; string-aware paren depth + POSITIONAL (head-only) keywords. A keyword colours only in
;; OPERATOR position -- the first form inside its enclosing paren. That's "this level has 0
;; forms so far", and the per-level form count is exactly what the sexp summary carries
;; (each `opens` entry is (bracket . (k+1)), k = children so far). So the entry form-count
;; stack -- "is the head slot already taken?" -- is read off the cached summary at a seam,
;; just like the in-string state and the depth.
;; Run:  racket scratch/keyword-highlight.rkt

(require "../rope-core.rkt"         ; make-summary make-rope multisect
         "../summaries.rkt"        ; bundle char-smr
         "../sexp-summary.rkt"     ; sexp-smr strsexp-smr strsexp-spines strsexp-in-string?
...
(define (analyze keywords doc i j)
  ...
  (define entry-in?     (strsexp-in-string? (strsexp-smr b)))
